...
struct horse_state {
    ...
    void get_grid() {
        ...
        for (int i = 0; i < 8; i++) {
            ...
            for (int j = 0; j < 8; j++) {
                position deep_new_pos = position(new_pos.x+direction[i].x, new_pos.y+direction[i].y);
                if (isValid(deep_new_pos) && route[deep_new_pos.x*6+deep_new_pos.y] == -1) {
                    child++;
                }
            }
...
void try_move() {
    ...
    for (int i = 0; i < avi_count; i++) {
        ...
        for (int j = i+1; j < avi_count; j++) {
            if (horse_states[j].deep_child > horse_states[max_count].deep_child) {
                max_count = j;
            } 
        }
